...
(() => {
    const byrValid = (x) => x >= 1920 && x <= 2002;
    const iyrValid = (x) => x >= 2010 && x <= 2020;
    const eyrValid = (x) => x >= 2020 && x <= 2030;
    const hgtValid = (x) => {
        if (!x) return false;
        const [value, unit] = x.match(/[0-9]+|cm|in+/g);
        const int = parseInt(value, 10);

        if (!value || !unit) return false;

        return unit === 'cm' ? int >= 150 && int <= 193 : int >= 59 && int <= 76;
    };
    const hclValid = (x) => /^#([a-fA-F0-9]{6})/g.test(x);
    const eclValid = (x) => /amb|blu|brn|gry|grn|hzl|oth/.test(x);
    const pidValid = (x) => /\b\d{9}\b/.test(x);

    const valid = input.filter(({
        byr, iyr, eyr, hgt, hcl, ecl, pid,
    }) => byrValid(byr) && iyrValid(iyr) && eyrValid(eyr) && hgtValid(hgt) && hclValid(hcl) && eclValid(ecl) && pidValid(pid));
